...
// The member "save recipe" write path. saved_recipes is the one member-write table
// (RLS: auth.uid() = user_id) — so the per-request auth-aware client writes directly,
// no service role. POST save/unsave; 303 back to the recipe (no-JS friendly) or JSON.
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  const { error } = await supabase
    .from("saved_recipes")
    .upsert({ user_id: user.id, recipe_slug: slug, note }, { onConflict: "user_id,recipe_slug" });
  if (error) return wantsJson ? json({ ok: false, error: error.message }, 500) : back("?saved=err");
